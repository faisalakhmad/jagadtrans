...
class Master_model extends CI_Model{   
    ...
    // Instansi
    var $column_search_instansi = array('instansi_nama'); 
    var $column_order_instansi = array(null, 'instansi_nama','instansi_keterangan');
    var $order_instansi = array('instansi_nama' => 'ASC');
    
    function get_instansi_datatable()
    {
        $get = $this->input->get();
        $this->_get_query_instansi();
        if($get['length'] != -1)
    ...
    private function _get_query_instansi()
    { 
        $get = $this->input->get();
        $this->db->from('master_instansi'); 
        $i = 0; 
        foreach ($this->column_search_instansi as $item)
        {
            if($get['search']['value'])
            { 
                if($i===0) 
                ...
                if(count($this->column_search_instansi) - 1 == $i) 
                    $this->db->group_end(); 
            }
            $i++;
        } 
        if(isset($get['order'])) 
        {
            $this->db->order_by($this->column_order_instansi[$get['order']['0']['column']], $get['order']['0']['dir']);
        } 
        else if(isset($this->order_instansi))
        {
            $order = $this->order_instansi;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }
 
    function count_filtered_datatable_instansi()
    {
        $this->_get_query_instansi();
        $query = $this->db->get();
    ...
    function count_all_datatable_instansi()
    {
        $this->db->from('master_instansi');
        return $this->db->count_all_results();
    }

    function rules_instansi()
    {
        return [
            [
            'field' => 'nama',
    ...
    function simpandata_instansi(){   
        ...
        $array = array(
            'instansi_nama'=>$post["nama"],
            'instansi_keterangan'=>$post["keterangan"] 
        );
        return $this->db->insert("master_instansi", $array);   
    } 

    function updatedata_instansi()
    {
        $post = $this->input->post();
        $this->instansi_nama = $post["nama"]; 
        $this->instansi_keterangan = $post["keterangan"]; 
        return $this->db->update("master_instansi", $this, array('instansi_id' => $post['idd']));
    } 

    function hapusdata_instansi()
    {
        $post = $this->input->post(); 
        $this->db->where('instansi_id', $post['idd']);
        return $this->db->delete('master_instansi');  
    }

    // End of INSTANSI ----------------------------------

    // Start of Jenis Dokumen ---------------------------
    var $column_search_jenis_dok = array('jdok_nama'); 
    var $column_order_jenis_dok = array(null, 'jdok_nama','jdok_keterangan');
    var $order_jenis_dok = array('jdok_nama' => 'ASC');
    
    function get_jenis_dok_datatable()
    {
        $get = $this->input->get();
        $this->_get_query_jenis_dok();
        if($get['length'] != -1)
        $this->db->limit($get['length'], $get['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    private function _get_query_jenis_dok()
    { 
        $get = $this->input->get();
        $this->db->from('master_jenis_dokumen'); 
        $i = 0; 
        foreach ($this->column_search_jenis_dok as $item)
        {
            if($get['search']['value'])
            { 
                if($i===0) 
                ...
                if(count($this->column_search_jenis_dok) - 1 == $i) 
                    $this->db->group_end(); 
            }
            $i++;
        } 
        if(isset($get['order'])) 
        {
            $this->db->order_by($this->column_order_jenis_dok[$get['order']['0']['column']], $get['order']['0']['dir']);
        } 
        else if(isset($this->order_jenis_dok))
        {
            $order = $this->order_jenis_dok;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }
 
    function count_filtered_datatable_jenis_dok(){
        $this->_get_query_jenis_dok();
        $query = $this->db->get();
        return $query->num_rows();
    }
 
    function count_all_datatable_jenis_dok()
    {
        $this->db->from('master_jenis_dokumen');
        return $this->db->count_all_results();
    }

    function rules_jenis_dok()
    {
        return [
            [
            'field' => 'nama',
    ...
    function simpandata_jenis_dok(){   
        $post = $this->input->post();   
        $array = array(
            'jdok_nama'=>$post["nama"],
            'jdok_keterangan'=>$post["keterangan"] 
        );
        return $this->db->insert("master_jenis_dokumen", $array);   
    } 

    function updatedata_jenis_dok()
    {
        $post = $this->input->post();
        $this->jdok_nama        = $post["nama"]; 
        $this->jdok_keterangan  = $post["keterangan"]; 
        return $this->db->update("master_jenis_dokumen", $this, array('jdok_id' => $post['idd']));
    } 

    function hapusdata_jenis_dok()
    {
        $post = $this->input->post(); 
        $this->db->where('jdok_id', $post['idd']);
        return $this->db->delete('master_jenis_dokumen');  
    }

    // End of Jenis Dokumen

    // Start of Jenis KECAMATAN ---------------------------
    var $column_search_kec  = array('kec_nama'); 
    var $column_order_kec   = array(null, 'kec_nama');
    var $order_jenis_kec    = array('kec_nama' => 'ASC');
    ...
    private function _get_query_kec()
    { 
        $get = $this->input->get();
        $this->db->from('master_kecamatan'); 
        $i = 0; 
        foreach ($this->column_search_kec as $item)
        {
            if($get['search']['value'])
            { 
                if($i===0) 
                ...
                if(count($this->column_search_kec) - 1 == $i) 
                    $this->db->group_end(); 
            }
            $i++;
        } 
        if(isset($get['order'])) 
        {
            $this->db->order_by($this->column_order_kec[$get['order']['0']['column']], $get['order']['0']['dir']);
        } 
        else if(isset($this->order_jenis_kec))
        {
            $order = $this->order_jenis_kec;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }
 
    function count_filtered_datatable_kec(){
        $this->_get_query_kec();
        $query = $this->db->get();
        return $query->num_rows();
    }
 
    function count_all_datatable_kec()
    {
        $this->db->from('master_kecamatan');
        return $this->db->count_all_results();
    }
    ...
    function rules_kec()
    {
        return [
            ...
            'rules' => 'is_unique[master_kecamatan.kec_nama]|required',
            ] 
        ];
    }

    function simpandata_kec(){   
        $post = $this->input->post();   
        $array = array(
            'kec_nama'=>$post["nama"]
        );
        return $this->db->insert("master_kecamatan", $array);   
    } 

    function updatedata_kec()
    {
        $post = $this->input->post();
        $this->kec_nama        = $post["nama"]; 
        return $this->db->update("master_kecamatan", $this, array('kec_id' => $post['idd']));
    } 

    function hapusdata_kec()
    {
        $post = $this->input->post(); 
        $this->db->where('kec_id', $post['idd']);
        return $this->db->delete('master_kecamatan');  
    }

    // DESA----------------------------------------------------------------------------------------
    var $column_search_desa  = array('desa_nama'); 
    var $column_order_desa   = array(null, 'desa_nama', kec_nama);
    var $order_jenis_desa    = array('desa_nama' => 'ASC');
    
    function get_data_desa()
    {
        $get = $this->input->get();
        $this->_get_query_desa();
        if($get['length'] != -1)
        $this->db->limit($get['length'], $get['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    private function _get_query_desa()
    { 
        $get = $this->input->get();
        $this->db->from('master_desa');
        $this->db->join('master_kecamatan', 'kec_id = desa_kec_id');

        $i = 0; 
        foreach ($this->column_search_desa as $item)
        {
            if($get['search']['value'])
            { 
                if($i===0) 
                ...
                if(count($this->column_search_desa) - 1 == $i) 
                    $this->db->group_end(); 
            }
            $i++;
        } 
        if(isset($get['order'])) 
        {
            $this->db->order_by($this->column_order_desa[$get['order']['0']['column']], $get['order']['0']['dir']);
        } 
        else if(isset($this->order_jenis_desa))
        {
            $order = $this->order_jenis_desa;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }
 
    function count_filtered_datatable_desa(){
        $this->_get_query_desa();
        $query = $this->db->get();
        return $query->num_rows();
    }
 
    function count_all_datatable_desa()
    {
        $this->db->from('master_desa');
        return $this->db->count_all_results();
    }
    ...
    function simpandata_desa(){   
        $post = $this->input->post();   
        $array = array(
            'desa_nama'=>$post["nama"],
            'desa_kec_id'=>$post["kecamatan"]
        );
        return $this->db->insert("master_desa", $array);   
    } 

    function updatedata_desa()
    {
        $post                   = $this->input->post();
        $this->desa_nama        = $post["nama"]; 
        $this->desa_kec_id      = $post['kecamatan'];
        return $this->db->update("master_desa", $this, array('desa_id' => $post['idd']));
    } 

    function hapusdata_desa()
    {
        $post = $this->input->post(); 
        $this->db->where('desa_id', $post['idd']);
