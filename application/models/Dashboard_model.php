...
class dashboard_model extends CI_Model{  
    ...
    public function graph_per_instansi(){
      return $this->db->select('kec_nama AS kecamatan,
          ...
          FROM master_kecamatan
          LEFT JOIN master_desa ON kec_id = desa_kec_id
          LEFT JOIN trans_pengiriman ON trans_desa_id = desa_id AND trans_status = "1" AND trans_tanggal = CURRENT_DATE() AND trans_instansi_id = "'.$this->session->userdata('id_instansi').'"
          GROUP BY kec_id
          ')->get()->result_array(); 
    }
