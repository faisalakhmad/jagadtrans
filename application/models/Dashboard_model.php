...
class dashboard_model extends CI_Model{  
    public function hariini_total_dok(){
        return $this->db->select('count(trans_id) as total')->from('trans_pengiriman')->where('DATE_FORMAT(trans_tanggal, "%Y-%m-%d") = CURDATE()')->get()->row(); 
    }
    ...
    public function penjualan_perbulan($bln)
    {
       return $this->db->select('SUM(total) as total')->from('penjualan')->where('DATE_FORMAT(tanggal, "%m") ="'. $bln.'"')->get()->row(); 
    }
    public function penjualan_perhari($tgl)
    {
       return $this->db->select('SUM(total) as total')->from('penjualan')->where('DATE_FORMAT(tanggal, "%Y-%m-%d") ="'. $tgl.'"')->get()->row(); 
    }
    public function laba_perbulan($bln)
    {
       return $this->db->select('SUM(total_laba) as total')->from('penjualan')->where('DATE_FORMAT(tanggal, "%m") ="'. $bln.'"')->get()->row(); 
    }
    public function laba_perhari($tgl)
    {
       return $this->db->select('SUM(total_laba) as total')->from('penjualan')->where('DATE_FORMAT(tanggal, "%Y-%m-%d") ="'. $tgl.'"')->get()->row(); 
    }

    public function laba_masuk($tgl)
    {
       return $this->db->select('SUM(total_laba) as total')->from('penjualan')->where('tanggal ="'. $tgl.'"')->get()->row(); 
    } 
    ...
    public function total_laba_hari_ini()
    {
       return $this->db->select('SUM(total_laba) as total')->from('penjualan')->where('tanggal ="'. date('Y-m-d').'"')->get()->row();
    } 
    ...
    public function laba_bulan_ini()
    {
       return  $this->db->select('SUM(total_laba) as total')->from('penjualan')->where('tanggal BETWEEN "'. date('Y-m-01'). '" and "'. date('Y-m-t').'"')->get()->row();
    } 
   ...
   public function get_produk_terlaris(){ 
     $this->db->select("SUM(a.kuantiti) as total, a.kode_item,  b.nama_item, b.satuan, b.stok");
